...
from keras.layers import Dense, Input, Embedding, Lambda, Dropout, Activation, SpatialDropout1D, Reshape, GlobalAveragePooling1D, merge, Flatten, Bidirectional, CuDNNGRU, add, Conv1D, GlobalMaxPooling1D, GRU
...
def get_av_pos_rnn(nb_words, embedding_dim, embedding_matrix, max_sequence_length, out_size):
    ...
    r1 = Bidirectional(GRU(recurrent_units, return_sequences=True))(embedding_layer)
    r1 = SpatialDropout1D(0.3)(r1)
    r2 = Bidirectional(GRU(recurrent_units, return_sequences=True))(r1)
